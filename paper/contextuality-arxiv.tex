...
\paragraph{Gap versus ratio.} The maximizer of $\theta/\alpha$ differs from the maximizer of $\gap$ at $n=8$ (a CHSH-type graph, \cite{tamer2026}), $n=10$ (\g{IUZurzmmo}, $\alpha=2$, $\theta=5/2$, ratio $1.25$) and $n=11$ (the dense graph in row~6 of Table~\ref{tab:n11}, ratio $1.2452$, versus $1.1937$ for the gap maximizer); at $n=9$ the two coincide.

\subsection{Triangle-free graphs, $n=12,13$}

Motivated by the $n=11$ result we searched exhaustively over connected triangle-free graphs, generated with \texttt{geng -t}~\cite{nauty}: $90{,}842$ graphs for $n=11$ (recovering the global top six), $1{,}144{,}061$ for $n=12$ and $19{,}425{,}052$ for $n=13$. The maxima are
\begin{align*}
...
\end{align*}
At $n=13$ the maximizer is isolated: no other connected triangle-free graph on 13 vertices reaches $\gap=0.95$ (the screen threshold), and among the triangle-free graphs with $\alpha\le5$ --- the $R(3,6)$-graphs on 13 vertices, all $275{,}086$ of which we solved exactly --- the best connected graph with $\alpha=5$ has $\gap=0.854480$. % TODO: replace by the full-class runner-up once the threshold-0.80 rerun completes
The maximizer is 4-regular with $|\mathrm{Aut}|=52$ and is isomorphic to the circulant graph $C_{13}(1,5)$, the unique $(3,5)$-Ramsey-critical graph~\cite{radziszowski}.
