...
\paragraph{Validation.} With no threshold, the certified bounds exceed $\theta$ by a median of $0.006$ (max $0.21$) over the $11{,}117$ graphs on eight vertices, and never fall below $\theta$. With thresholds $0.60$ at $n=9$ and $0.65$ at $n=10$, the survivors are exactly the two, respectively thirty, graphs known to exceed the threshold (from the exact runs and from~\cite{danielsen-db}), with no misses and no false survivors.
...
All scripts (exact search, GPU screening, Ramsey catalogue processing), the result tables with graph6 identifiers, and the certificates for the record graphs are available at \texttt{<repository URL>}. % TODO

\appendix
\section{Reproduction of the eight-vertex ranking}\label{app:n8}
% TODO: table matching Tamer et al. Table II (ranks 1-10) from results/n8.csv

\section{graph6 strings and certificates}\label{app:g6}
% TODO: full graph6 of the R(3,8) record; edge lists of the record graphs; dual certificates
