...
\begin{abstract}
In the exclusivity-graph approach to contextuality, a noncontextuality inequality is a graph $G$, its noncontextual bound is the independence number $\alpha(G)$ and its quantum bound is the Lov\'asz number $\theta(G)$. The absolute contextuality gap $\gap(G)=\theta(G)-\alpha(G)$ was recently maximized exhaustively over the $11{,}117$ connected graphs on eight vertices (Quad-$C_5$, $\gap=0.46784$). We extend the exhaustive search to nine, ten and eleven vertices --- $261{,}080$, $11{,}716{,}571$ and $1{,}006{,}700{,}565$ connected graphs respectively --- and find $\gap_{\max}=2/3$ (with $\theta=11/3$), $\gap_{\max}=1/\sqrt2$ (with $\theta=3+1/\sqrt2$) and $\gap_{\max}=0.774889$. The search over $10^9$ graphs was made feasible on a single consumer GPU by a certified screening scheme: for each graph a feasible point of the dual semidefinite program is an explicit upper bound on $\theta$, found by numerical optimization but valid as a mathematical certificate, and only graphs whose certified bound can beat the running record are solved exactly. The maximizers are not a nested family, and at eleven vertices a regime change occurs: the dense, Wagner-based maximizers with $\alpha=3$ are overtaken by sparse triangle-free graphs with $\alpha=4$. We read this through Ramsey theory, as a tension between two invariants: triangle-free graphs with $\alpha\le k-1$ are exactly the $R(3,k)$-graphs, which exist only on fewer than $R(3,k)$ vertices, so Ramsey numbers hold $\alpha$ down while $\theta$ grows with $n$. The record graphs at the Ramsey-critical scales are $R(3,k)$-graphs --- Quad-$C_5$ and the Wagner graph are $R(3,4)$-graphs, the eleven-vertex maximizer is an $R(3,5)$-graph, and the triangle-free maximizer on thirteen vertices is the unique $R(3,5)$-critical graph $C_{13}(1,5)$, with $\theta=13(-\lambda_{\min})/(\lambda_{\max}-\lambda_{\min})=5.181737$ in closed form --- while the intermediate maximizers on nine and ten vertices retain the previous independence number by admitting triangles. Computing $\theta$ over McKay's complete catalogues of $R(3,k)$-graphs yields a ladder of lower bounds on $\gap_{\max}(n)$ up to $n=35$ ($\gap\ge 4.4106$ from the unique $R(3,9)$-critical graph, Kalbfleisch's circulant $C_{35}(1,7,11,16)$). On the theoretical side we prove $\gap(G)\le\nu(G)\le n/2$ for every graph, that the gap is superadditive so that $c^*=\lim\gap_{\max}(n)/n$ exists with $0.126\le c^*\le1/2$, and consequently $\gap_{\max}(R(3,k)-1)=\Omega(k^2/\ln k)$. We release all code, graph6 identifiers and certificates.
\end{abstract}
...
\end{quote}
It is verified over all graphs at $n=8$ ($k=4$, \cite{tamer2026}) and, in the triangle-free class, at $n=13$ ($k=5$); for the other critical sizes ($17,22,27,35$) we can only report the maximum over the critical family itself.
...
For $n=R(3,k)-1$ with $k\ge4$, $\gap_{\max}(n)$ is attained by an $R(3,k)$-critical graph; more generally, from $n=11$ on the maximizer is triangle-free with the least independence number that Ramsey theory allows.
\end{conjecture}
...
\paragraph{The certificate.} For any graph $G$ and \emph{any} symmetric matrix $Y$ with $Y_{ij}=0$ whenever $ij\notin E$ (diagonal included), weak duality gives $\theta(G)\le\lambda_{\max}(J+Y)$. Thus a pair $(Y,\lambda)$ with $\lambda\ge\lambda_{\max}(J+Y)$ is a proof that $\theta(G)\le\lambda$, and if moreover $\lambda-\alpha(G)<\gap_{\rm thr}$ then $G$ cannot beat the threshold. Nothing about how $Y$ was found enters this statement. In our implementation $Y$ is a single-precision matrix whose support is enforced exactly by masking, so it is an exactly known rational matrix; the only numerical step in the certificate is the evaluation of $\lambda_{\max}(J+Y)$, which we do with a backward-stable symmetric eigensolver. For an $11\times11$ symmetric matrix with entries bounded by $3$ in magnitude, the computed largest eigenvalue differs from the exact one by at most $c\,n\,u\|J+Y\|_2\lesssim 10^{-5}$ in single precision ($u=2^{-24}$, $c$ a small constant), so $\lambda:=\lambda_{\max}^{\rm computed}+10^{-3}$ is a valid upper bound with two orders of magnitude to spare; the survivors' final certificates are recomputed in double precision, where the margin is $10^{9}$ times the error.
